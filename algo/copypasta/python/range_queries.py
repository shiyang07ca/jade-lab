"""Immutable matrix sums and batch range additions."""
...
class MatrixPrefixSum:
    ...
    def __init__(self, matrix: Sequence[Sequence[int]]) -> None:
        ...
        self._prefix = [[0] * (columns + 1) for _ in range(rows + 1)]
        for row_index, row in enumerate(matrix):
            for column_index, value in enumerate(row):
                self._prefix[row_index + 1][column_index + 1] = (
                    self._prefix[row_index + 1][column_index]
                    + self._prefix[row_index][column_index + 1]
                    - self._prefix[row_index][column_index]
                    + value
                )
...
def apply_range_additions(length: int, updates: Sequence[tuple[int, int, int]]) -> list[int]:
    ...
    for update in updates:
        ...
        if any(
            not isinstance(value, int) or isinstance(value, bool) for value in (left, right, delta)
        ):
