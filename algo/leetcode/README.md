...
仓库根目录的 `mise.toml` 已锁定 Python 3.13.5 和 leetgo 1.4.17。新机器先安装 `mise`，然后在仓库根目录执行：
...
```sh
cd algo/leetcode
mise exec -- uv sync --frozen --python 3.13.5
...
如果 `.venv` 被 leetgo 重建，重新执行 `mise exec -- uv sync --frozen --python 3.13.5`，让本地运行环境回到
...
```sh
cd algo/leetcode
uv run --frozen python check.py 88
```

Python 解法会运行 `basedpyright`、Ruff 和已提交的 pytest；Go 解法会运行 `gofmt` 和已提交的 Go 测试。
没有提交语言测试的题目仍回退到 LeetGo 本地测试。该命令不会扫描其他题目、访问远程判题或提交代码。
