...
| 软件包 | `packages/<name>/` | 代码调用方 | 稳定、可独立消费的依赖单元；公开接口与消费方测试持续通过 |
...
软件包的主要接口面向代码调用方，目标是让现有模块或新项目直接依赖。它不要求仓库内已经有生产调用方，但必须有
具体复用场景和隔离的消费方测试，不能只凭“以后可能有用”晋升。一个实现进入 `packages/` 前必须满足：

1. README 指出至少一个具体调用场景、提供的能力和不负责的内容。
2. 使用方只通过公开类型、函数或模块调用，不依赖内部文件布局。
3. 输入限制、返回值、可观察副作用和错误类型明确；外部输入在入口验证。
4. 测试像外部项目一样只从公开接口调用，覆盖正常路径、重要边界和主要错误。
...
`labs/`，不能把 `.scratch/` 当作低成熟度源码目录。只有接口或进程使用方式稳定后才决定后续去向：

- 现有或明确的新项目场景需要直接依赖它：整理为 `packages/<name>/`，补公开接口、隔离消费方测试和构建产物。
